    ...
    passport.use(
        ...
        function(req, username, password, done) { // callback with email and password from our form
            connection.query("SELECT * FROM users WHERE name = ?",[username], function(err, rows){
                ...
                !bcrypt.compareSync(password, rows[0].password, function(err, res){
                    if(!err){
                        return done(null, false, req.flash('loginMessage', 'Все круто!!!')); // create the loginMessage and save it to session as flashdata
                    }else{
